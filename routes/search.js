import { findFlightsForDate, findFlightsInMonth } from "utils/api.js";

function searchFlights(params) {
  const month = params.get("month[id]");
  const monthSearch = Boolean(month);
  const regionFrom = params.get("region_from")
    ? JSON.parse(params.get("region_from"))
    : null;
  const regionTo = params.get("region_to")
    ? JSON.parse(params.get("region_to"))
    : null;
